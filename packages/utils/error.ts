export class DomainError extends Error {
	...
	static forbidden(message = "You don't have enough permissions to perform this action"): DomainError {
		return new DomainError(message, 403);
	}

	static notFound(message = 'The resource you requested does not exist'): DomainError {
		return new DomainError(message, 404);
	}

	static unprocessableEntity(message = 'We are unable to process this request'): DomainError {
